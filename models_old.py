...
class ConvPatchEmbedding(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=96):
        super().__init__()
        self.main = nn.Sequential(
            ConvBlock(in_chans, 12),
            ConvBlock(12, 24, stride=2),
            ConvBlock(24, 48),
            ConvBlock(48, 96, stride=2),
            ConvBlock(96, 192),
            ConvBlock(192, 192, stride=2),
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224, 224), in_chans=3,
                 embed_dim=96, depths=[2, 2, 4, 2], num_heads=[3, 6, 9, 12],
        ...
        num_patches = img_size[0] * img_size[1] * img_size[2] // 512
        patches_resolution = [img_size[0] // 8, img_size[1] // 8, img_size[2] // 8]
...
class Final(nn.Module):
    ...
    def __init__(self, embed_dim):
        ...
        self.embdedding_conv2 = nn.Sequential(
            nn.AvgPool3d(kernel_size=2, stride=2),
            ConvBlock(2, embed_dim // 2))
        self.embdedding_conv3 = ConvBlock(2, embed_dim // 4)
        self.up = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False)
        self.conv1 = ConvBlock(embed_dim * 2, embed_dim // 2)
        self.conv2 = ConvBlock(embed_dim, embed_dim // 4)
        self.conv3 = ConvBlock(embed_dim // 2, embed_dim // 8)
        self.head = RegistrationHead(embed_dim // 8)

    def forward(self, cross, x, y):
        xy = torch.cat([x, y], dim=1)
        cross = self.conv1(torch.cat([self.up(cross), self.embdedding_conv1(xy)], dim=1))
        cross = self.conv2(torch.cat([self.up(cross), self.embdedding_conv2(xy)], dim=1))
        cross = self.conv3(torch.cat([self.up(cross), self.embdedding_conv3(xy)], dim=1))
...
class PUA(nn.Module):
    ...
    def __init__(self, img_size=(224, 224, 224), embed_dim=48, depths=[2, 18, 2], num_heads=[16, 32, 64], window_size=(7, 7, 7)):
        super().__init__()
